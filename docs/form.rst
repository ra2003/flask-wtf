...
``None``. ::
    ...
    from flask_wtf import FlaskForm
    from flask_wtf.file import MultipleFileField, FilesRequired
    from werkzeug.utils import secure_filename

    class PhotoForm(FlaskForm):
        photos = MultipleFileField(validators=[FilesRequired()])
...
Similarly, you can use :class:`FilesRequired` and :class:`FilesAllowed` for
multiple files validation. They also can be used with both
Flask-WTF's and WTForms's ``MultipleFileField`` classes.
